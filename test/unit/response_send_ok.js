...
test('response', function (t) {
    ...
    response.writeHead = function (code, headers) {
        ...
        t.equal(headers['Content-Type'], 'text/plain', 'expected content type');
        t.equal(headers['Content-Length'], 11, 'expected content length');
    };

    response.end = function (body) {
        t.type(body, 'string', 'body is of expected type');
        t.equal(body, 'hello world', 'body is of expected value');
        t.end();
    };
